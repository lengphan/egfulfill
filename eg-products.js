...
function renderOpProducts() {
  ...
  grid.innerHTML = filtered.map(p => {
    ...
    <div class="card" style="overflow:hidden;cursor:pointer;transition:border-color .15s,box-shadow .15s;position:relative;display:flex;flex-direction:column;padding:0" onclick="openProductCard(${p.id})" onmouseover="this.style.borderColor='#9ca3af'" onmouseout="this.style.borderColor='#e5e4e0'">
      ...
      <div style="background:#fff;height:285px;overflow:hidden;border-bottom:1px solid #e5e4e0;display:flex;align-items:center;justify-content:center">
        <img src="${p.img}" loading="lazy" style="width:100%;height:100%;object-fit:contain;object-position:center;transform:scale(1.5);display:block" onerror="this.onerror=null;this.src='https://placehold.co/400x400/f0ede9/6b7280?text=${p.fallback}'"/>
      </div>
