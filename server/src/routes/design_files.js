...
export function designFilesRoutes(app, requireAuth) {
  ...
  q('alter table design_file_data add column if not exists url text').catch(() => {});
  ...
  app.post('/api/design_files', { preHandler: requireAuth }, async (req, reply) => {
    ...
    await q(
      `insert into design_file_data (design_id, order_id, sku, seller_id, file_name, mime, data, url, content_hash, created_at, updated_at)
  ...
  app.get('/api/design_files/:designId', { preHandler: requireAuth }, async (req, reply) => {
    const r = await q('select design_id, order_id, sku, seller_id, file_name, mime, data, url from design_file_data where design_id=$1', [String(req.params.designId)]);
    const row = r.rows[0];
