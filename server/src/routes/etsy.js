...
export function etsyRoutes(app, requireAuth, requireStaff) {
  ...
  app.get('/api/etsy/connections', { preHandler: requireAuth }, async (req) => {
    ...
    const r = await q(
      staff ? `select id, platform, shop_id, shop_name, scopes, last_sync_at, created_at, backfill_days, token_expires_at,
            ...
            : `select id, platform, shop_id, shop_name, scopes, last_sync_at, created_at, backfill_days, token_expires_at,
                        (select min(o.created_at) from orders o where o.source='etsy' and o.seller_id = pc.connected_by) as oldest_order_at
  ...
  app.post('/api/etsy/exchange', { preHandler: requireAuth }, async (req, reply) => {
    ...
    try {
      ...
      audit(req, 'etsy.connected', { entityType: 'connection', entityId: String(shopId),
        after: { platform: 'etsy', shop_id: String(shopId), shop_name: shopName } });
  ...
  app.delete('/api/etsy/connections/:shop_id', { preHandler: requireAuth }, async (req) => {
    ...
    const was = (await q(
      `select shop_name from platform_connections where platform='etsy' and shop_id=$1`,
    ...
    audit(req, 'etsy.disconnected', { entityType: 'connection', entityId: String(req.params.shop_id),
      before: { platform: 'etsy', shop_id: String(req.params.shop_id), shop_name: was.shop_name || null } });
    return { ok: true };
  });
