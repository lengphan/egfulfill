...
export function DesignMaker() {
 ...
 className="w-full"
               ...
               *
               * This was `faceUrl || mockup`, and `mockup` is the product's FRONT image. So a
               * side the product has no picture for drew the front garment while the switcher
               * above still read "Back" — you positioned a back print against a front photo,
               * against the front's print zone, and nothing said so. The comment on `side`
               * fifty lines up says the side must drive both the image and the zone; this
               * line quietly undid it.
               *
               * faceUrl already makes that distinction (it falls back to the type outline on
               * FRONT only, and to nothing elsewhere). So the same rule here: `mockup` is a
               * front fallback, for a product whose imagery resolves through mockupOf and not
