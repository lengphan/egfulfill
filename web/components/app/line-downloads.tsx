 ...
 */
export function LineDownloads({ design, files, item, sides }: {
  design?: OrderDesign | null
  files?: DesignFileRow[]
  item: { line_id?: string | null; sku?: string | null; name?: string | null }
  /** Every position's artwork, keyed by side — the map the Files tab reads. */
  sides?: Record<string, OrderDesign>
}) {
  const tl = useLabelT()
  const [busy, setBusy] = useState<string | null>(null)
  /**
   * EVERY POSITION, NOT JUST THE FRONT (2026-09-07). A three-position line offered one
   * download — the front, which is what the singular `design` holds — and the back and
   * sleeve files were reachable only through the designer. One entry per side with art,
   * in the order the sides are printed; the singular stays as the fallback for a line the
   * per-side map has not been loaded for.
   */
  const sideArts = Object.entries(sides ?? {})
    .map(([side, d]) => ({ side: side.toLowerCase(), design: d, src: designSrc(d?.data) }))
    .filter((a) => !!a.src)
    .sort((a, b) => SIDE_ORDER.indexOf(a.side) - SIDE_ORDER.indexOf(b.side))
  const arts = sideArts.length
    ? sideArts
    : (() => { const src = designSrc(design?.data); return src ? [{ side: "", design: design ?? undefined, src }] : [] })()
  const mine = filesForLine(files, { line_id: item.line_id, sku: item.sku })
  const machine = mine.filter((f) => f.kind === "emb" || f.kind === "pes")
  if (!arts.length && !machine.length) return null

  const stem = (item.name || item.sku || "design").replace(/[^a-z0-9]+/gi, "-").slice(0, 40)

  /**
   * THE FILE'S OWN NAME, per row, the same as the stitch-file row beside it — and WHAT IT
   * ACTUALLY IS rather than a hardcoded "PNG": the badge is the fact you scan for when a
   * line carries a JPG the printer will refuse. Read from the name, then the URL, then the
   * data: URI's own mime type.
   */
  ...
  const count = arts.length + machine.length
  const extOf = (name: string, src: string) => {
    const e = extFrom(name) || extFrom(src.startsWith("data:") ? "" : src)
      || (/^data:image\/([a-z0-9.+-]+)/i.exec(src)?.[1] ?? "").toUpperCase() || "PNG"
    return e === "JPEG" ? "JPG" : e
  }
