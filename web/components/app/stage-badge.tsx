...
export function StageBadge({ status }: { status?: string | null }) {
  ...
  if (!id) return <span className={STATUS_TONE.settled}>{tl("stage", "Draft")}</span>
  const m = stageMeta(id)
  return <span className={m ? TONE_CLASS[m.tone] : STATUS_TONE.settled}>{tl("stage", m?.label ?? id)}</span>
}
