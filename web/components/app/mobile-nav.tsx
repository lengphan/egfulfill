  ...
  return (
    <>
      <div className="sticky top-0 z-30 flex h-14 items-center gap-3 border-b border-border bg-card px-4 md:hidden">
        <button onClick={() => setOpen(true)} aria-label="Open menu" className="flex size-9 items-center justify-center rounded-lg text-foreground hover:bg-accent">
        ...
        <div className="fixed inset-0 z-50 md:hidden" role="dialog" aria-modal="true">
          ...
          <div className="absolute inset-y-0 left-0 flex w-72 max-w-[82%] flex-col border-r border-border bg-card shadow-xl">
            <div className="flex h-14 shrink-0 items-center justify-between border-b border-border px-4">
              <span className="font-display text-xl font-semibold tracking-tight">egfulfill</span>
              <button onClick={() => setOpen(false)} aria-label="Close menu" className="flex size-9 items-center justify-center rounded-lg text-muted-foreground hover:bg-accent">
            ...
            <nav className="flex-1 overflow-y-auto p-3">
              {sections.map((section, i) => (
                <div key={i} className="mb-1">
                  {section.heading && <div className="px-3 pb-2 pt-5 text-[10.5px] font-semibold uppercase tracking-wider text-muted-foreground">{section.heading}</div>}
            ...
            <div className="shrink-0 border-t border-border p-3">
              <button onClick={() => { setOpen(false); onLogout() }} className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-foreground/70 hover:bg-accent hover:text-foreground">
                <SignOut size={20} className="text-muted-foreground" /> Log out
              </button>
            </div>
          </div>
        </div>
