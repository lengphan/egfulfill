...
export function SupplierTerms({ row, onSaved }: { row: SourcingRow; onSaved?: () => void }) {
    ...
    <div className="grid gap-4 border-t border-border p-4 lg:grid-cols-2">
      ...
      <div className="space-y-2">
        <h3 className="flex items-center gap-1.5 text-xs font-semibold">
          <Handshake size={14} weight="bold" className="text-primary" /> {tl("supplierTerms", "Agreed terms")}
        </h3>
 ...
 source for those. These are the two that had nowhere to live. */}
        <p className="text-2xs text-muted-foreground">
          {tl("supplierTerms", "What they committed to, as opposed to what the listing says. Unit price, MOQ and lead time stay on the row above.")}
        </p>
        <div className="grid gap-2 sm:grid-cols-2">
          <label className="text-xs">
            <span className="text-muted-foreground">{tl("supplierTerms", "Sample cost")}</span>
 ...
 value={sampleCost} onChange={(e) => setSampleCost(e.target.value.replace(/[^0-9.]/g, ""))} />
          ...
          <label className="text-xs">
            <span className="text-muted-foreground">{tl("supplierTerms", "Payment terms")}</span>
 ...
 value={terms} onChange={(e) => setTerms(e.target.value)} />
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button size="sm" onClick={() => void save(true)} disabled={saving}>
            {tl("supplierTerms", "Save + mark confirmed today")}
          </Button>
        </div>
 ...
 reads as something to re-ask, which is the decision this is for. */}
        <p className="text-2xs text-muted-foreground">
          {stale
            ? <>Last confirmed {stale.label} — <span className={stale.days > 90 ? "font-medium text-hold" : ""}>
                {stale.days === 0 ? "today" : `${stale.days} day${stale.days === 1 ? "" : "s"} ago`}
              </span>{stale.days > 90 ? tl("supplierTerms", ". Worth re-asking before you commit to a buy.") : ""}</>
 : tl("supplierTerms", "Never confirmed — these are quoted figures, not agreed ones.")}
        </p>
      ...
      <div className="space-y-2">
        <h3 className="flex items-center gap-1.5 text-xs font-semibold">
          <ChatText size={14} weight="bold" className="text-primary" /> {tl("supplierTerms", "What they said")}
        </h3>
        <p className="text-2xs text-muted-foreground">
          {tl("supplierTerms", "Paste it. Alibaba’s API exposes no messages, so this is the only copy — kept word for word rather than summarised.")}
        </p>
 ...
 rows={3}
 placeholder={tl("supplierTerms", "Paste what they wrote — price, MOQ, lead time, whatever they committed to.")}
 ...
 className="eg-select h-8 rounded-md border border-border bg-card px-2 text-xs">
            <option value="them">{tl("supplierTerms", "They said")}</option>
            <option value="us">{tl("supplierTerms", "We said")}</option>
          </select>
 ...
 className="h-8 w-40 text-xs" title={tl("supplierTerms", "The day it was said — defaults to today")} />
        ...
        {msgs === null ? (
          <div className="py-3 text-center text-2xs text-muted-foreground">
            <CircleNotch size={13} className="mr-1 inline animate-spin" /> {tl("supplierTerms", "Loading…")}
          </div>
        ) : msgs.length === 0 ? (
          <p className="py-3 text-center text-2xs text-muted-foreground">{tl("supplierTerms", "Nothing recorded yet.")}</p>
        ) : (
              ...
              <li key={m.id} className="group rounded-xl border border-border bg-card p-2">
                <div className="flex items-center gap-2 text-2xs text-muted-foreground">
                  <span className={"rounded px-1.5 py-0.5 font-medium " + (m.direction === "us"
                    ...
                    {m.direction === "us" ? tl("supplierTerms", "We said") : tl("supplierTerms", "They said")}
                  </span>
                  <span>{m.saidAt ?? tl("supplierTerms", "no date")}</span>
                  <button onClick={() => void drop(m)}
 className="ml-auto opacity-0 transition-opacity group-hover:opacity-100 hover:text-destructive"
 title={tl("supplierTerms", "Delete this entry")}>
                    <Trash size={12} />
                  </button>
                </div>
