                ...
                <tbody>
                  {data.campaigns.map((c) => (
                    <tr key={`${c.channel}-${c.id}`} className="border-t border-border">
                      ...
                      <td className="px-4 py-2.5"><span className={"rounded-full px-2 py-0.5 text-2xs font-semibold uppercase " + (CHANNEL[c.channel]?.cls ?? "bg-muted")}>{CHANNEL[c.channel]?.label ?? c.channel}</span></td>
