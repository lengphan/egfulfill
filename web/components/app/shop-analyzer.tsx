...
export function ShopAnalyzer() {
 ...
 return (
      <div className="flex flex-col items-center gap-3 py-20 text-center">
        ...
        <div className="font-medium">{tl("shopAnalyzer", "No Etsy shop connected")}</div>
        <p className="max-w-sm text-sm text-muted-foreground">{tl("shopAnalyzer", "Connect your Etsy shop and we’ll analyze your listings, pricing and tags, then tell you what to fix first.")}</p>
        <Link href="/stores"><Button size="sm" className="mt-1">{tl("shopAnalyzer", "Connect a shop")}</Button></Link>
      </div>
    )
  }
    ...
    <div className="space-y-4 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <span className="text-base font-bold">{data?.shop?.shop_name || tl("shopAnalyzer", "Your shop")}</span>
            {data?.shop?.url && (
          ...
          <div className="text-xs text-muted-foreground">
            {data?.at ? `Last analyzed ${when(data.at)}` : tl("shopAnalyzer", "Not analyzed yet")}
            {data?.cached && tl("shopAnalyzer", " · cached")}
          </div>
        </div>
        <Button size="sm" onClick={() => run(!!data?.stats)} disabled={running}>
          ...
          {running ? tl("shopAnalyzer", "Analyzing…") : data?.stats ? tl("shopAnalyzer", "Re-analyze") : tl("shopAnalyzer", "Analyze my shop")}
        </Button>
      </div>
      ...
      {!s && !running && !err && (
        <div className="flex flex-col items-center gap-2 py-16 text-center text-muted-foreground">
          ...
          <div className="text-sm">{tl("shopAnalyzer", "Run an analysis to see how your shop is doing and what to fix.")}</div>
        </div>
      )}
      ...
      {s && (
        <>
          ...
          {sales ? (
            <StatGrid>
              <StatCard label={tl("shopAnalyzer", "Revenue")} value={usd(sales.revenue)} sub={`last ${sales.windowDays}d · real`} tone="pos" />
              <StatCard label={tl("shopAnalyzer", "Orders")} value={String(sales.orders)} sub={`last ${sales.windowDays}d · real`} />
              <StatCard label={tl("shopAnalyzer", "Avg order")} value={usd(sales.avgOrderValue)} sub="real" />
              <StatCard label={tl("shopAnalyzer", "Active listings")} value={String(s.listingCount)} sub={`${s.totalFavorites.toLocaleString()} favorites`} />
            </StatGrid>
          ) : (
            <StatGrid>
              <StatCard label={tl("shopAnalyzer", "Active listings")} value={String(s.listingCount)} sub={tl("shopAnalyzer", "on Etsy")} />
              <StatCard label={tl("shopAnalyzer", "Favorites")} value={s.totalFavorites.toLocaleString()} sub={tl("shopAnalyzer", "all listings")} />
              <StatCard label={tl("shopAnalyzer", "Median price")} value={`$${s.medianPrice}`} sub={s.priceRange ? `$${s.priceRange.min}–$${s.priceRange.max}` : "—"} />
              <StatCard label={tl("shopAnalyzer", "Est. revenue")} value={usd(s.estRevenue)} sub={tl("shopAnalyzer", "all time · estimate")} />
            </StatGrid>
          )}
          ...
          {!sales && (
            <div className="flex items-start gap-1.5 rounded-lg border border-border px-3 py-2 text-xs text-muted-foreground">
              ...
              <span>{tl("shopAnalyzer", "No synced Etsy orders yet, so sales are")} <b>estimated</b> {tl("shopAnalyzer", "from favorites & listing age. Etsy publishes no views or conversion data — once your orders sync, this switches to your real numbers.")}</span>
            </div>
          )}

          {/* Fixable, countable problems — the concrete half of the advice. */}
          <SectionCard title={tl("shopAnalyzer", "Listing health")}>
            <div className="grid grid-cols-2 gap-px bg-border sm:grid-cols-4">
              {[
                { label: tl("shopAnalyzer", "Missing tags"), n: s.issues.noTags, hint: tl("shopAnalyzer", "no tags at all") },
                { label: tl("shopAnalyzer", "Thin tags"), n: s.issues.thinTags, hint: tl("shopAnalyzer", "under 13 of 13") },
                { label: tl("shopAnalyzer", "Short titles"), n: s.issues.shortTitles, hint: tl("shopAnalyzer", "under 40 chars") },
                { label: tl("shopAnalyzer", "One image"), n: s.issues.singleImage, hint: tl("shopAnalyzer", "fewer than 2") },
              ].map((it) => (
          ...
          {sales && sales.topSellers.length > 0 && (
            <SectionCard title={tl("shopAnalyzer", "Best sellers")}>
          ...
          {data?.advice && (
            <SectionCard title={<span className="flex items-center gap-1.5"><Sparkle size={14} weight="fill" className="text-primary" /> {tl("shopAnalyzer", "Recommendations")}</span>}>
