...
- **SanMar catalog is a FILE, not an API call.** SanMar gives you two doors and the guide is explicit about which to use: the bulk SDL over SFTP (`ftp.sanmar.com:2200`, folder `SanMarPDD`) for the catalog, and SOAP only for live inventory/pricing. Never walk the catalog through SOAP.
  - **[tools/sanmar-sync.sh](tools/sanmar-sync.sh)** fetches `SanMar_SDL_N.zip` (~7MB → 195MB CSV) over SFTP, unzips to `/root/sanmar/sdl`, then calls `POST /api/sanmar/import/local`, which reads it off a read-only bind mount (`SANMAR_DATA_DIR`). **The browser can never carry this file** — 195MB against a 60MB body limit and Vercel's ~4.5MB proxy cap.
    - **It lives in the repo now, and that is the fix.** The old one was `/root/sanmar/sync.sh`, host-only, and died with the box on 2026-08-26 — no copy, no history. The catalogue data survived inside the pg_dump; the thing that refreshes it did not, and nobody noticed for three weeks (last import 2026-08-25, found 2026-09-14). Install with `cp tools/sanmar-sync.sh /root/` + a cron line; the header carries the steps.
    - **It mints its own 5-minute admin JWT inside the api container.** The import route is admin-only and sessions expire after 7 days (`auth.js`), so a static token in a cron job dies every week and the sync then fails silently. The container signs one with the `JWT_SECRET` it already holds — no stored password, and `sub` reads `sanmar-sync` rather than impersonating a person. It runs there rather than curling from the host because `curl localhost:3000` has never worked on this box.
    - **The FTP credentials are NOT the API credentials.** SanMar issue them separately, over a one-time Bitwarden Send link that expires in 7 days. The API trio goes in Settings › Integrations (it is on the `app_secrets` allow-list, so it survives a rebuild); `SANMAR_FTP_USER` / `SANMAR_FTP_PASS` are read only by the sync script, so they live in `.env`.
